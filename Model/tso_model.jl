...
function Bellman_coll_npar(prim::Primitives, prim_grp::Primitives_collect, param::Params, res::Results_npar)
    ...
    for i_x = 1:nX, i_χ = 1:nχ
        ...
        for i_m = 1:nm #loop over major selection. We doing anything with priors of teaching ability?
            val = γ_eul + Π * log(sum(exp.(v_work_a[i_x, i_χ, i_m, 1, 1, 1, 1, 1, 1, :]))) #expected value
...
function Bellman_a_npar(prim::Primitives, prim_grp::Primitives_collect, param::Params, res::Results_npar, t::Int64)
    ...
    #begin main loop over state space
    for i_x = 1:nX, i_χ = 1:nχ, i_m = 1:nm, i_MA = 1:nMA, i_l = 1:nl, i_e = 1:ne, i_ξ = 1:nξ, i_d = 1:ndt
        ...
        if MA == 1 #no choice to be made
            val = γ_eul + log(sum(exp.(v_work_b[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, t, :]))) #expected value

            if l == 1 #already have a license
                val = v_work_b[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, t, 1]
            end

            res.v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, t, :] .= val
        elseif MA == 0 #now we're making a choice
            for i_s = 1:2 #1 = no masters. 2 = masters
                val = γ_eul + log(sum(exp.(v_work_b[i_x, i_χ, i_m, i_s, i_l, i_e, i_ξ, i_d, t, :]))) #expected value

                if l == 1
                    val = v_work_b[i_x, i_χ, i_m, i_s, i_l, i_e, i_ξ, i_d, t, 1]
                end

                cost = cost_MA(prim, param, Ω) * (i_s-1) #cost of obtaining masters
                res.v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, t, i_s] = val - cost
            end
        end
    end
end
...
function Bellman_b_npar(prim::Primitives, prim_grp::Primitives_collect, param::Params, res::Results_npar, t::Int64)
    ...
    for i_x = 1:nX, i_χ = 1:nχ, i_m = 1:nm, i_MA = 1:nMA, i_l = 1:nl, i_e = 1:ne, i_ξ = 1:nξ, i_d = 1:ndt
        ...
        if l == 1 #no choice to be made
            ...
            val = γ_eul + μ_j * log(sum(exp.(v_work_d[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, 2, t, 1:J+1]))) #expected value, offer
            val += (1-μ_j) * log(sum(exp.(v_work_d[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, 1, t, 1:J]))) #expected value, no offer
            res.v_work_b[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, t, :] .= val
        elseif l == 0 #now we're making a choice
            for i_s = 1:2 #1 = no license. 2 = license
                ...
                val = γ_eul + μ_j * log(sum(exp.(v_work_d[i_x, i_χ, i_m, i_MA, i_s, i_e, i_ξ, i_d, 2, t, 1:J+1]))) #expected value, offer
                val += (1-μ_j) * log(sum(exp.(v_work_d[i_x, i_χ, i_m, i_MA, i_s, i_e, i_ξ, i_d, 1, t, 1:J]))) #expected value, no offer
                cost = cost_license(prim, param, Ω) * (i_s-1) #cost of obtaining masters
                res.v_work_b[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, t, i_s] = val - cost
            end
        end
    end
end
...
function Bellman_d_npar(prim::Primitives, prim_grp::Primitives_collect, param::Params, res::Results_npar, t::Int64)
    ...
    if t == T #in terminal period
        ...
        for i_x = 1:nX, i_χ = 1:nχ, i_m = 1:nm, i_MA = 1:nMA, i_l = 1:nl, i_e = 1:ne, i_ξ = 1:nξ, i_d = 1:ndt, i_𝒥 = 1:n𝒥
            ...
            for i_j = 1:J #loop over occupation choices
                ...
                res.v_work_d[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, i_𝒥, t, i_j+1] = val #update
            end
            res.v_work_d[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, i_𝒥, t, 1] = 0.0 #update home work option\
        end
    elseif t!=T #not in terminal period
        ...
        for i_x = 1:nX, i_χ = 1:nχ, i_m = 1:nm, i_MA = 1:nMA, i_l = 1:nl, i_e = 1:ne, i_ξ = 1:nξ, i_d = 1:ndt, i_𝒥 = 1:n𝒥
            ...
            for i_j = 1:J #loop over occupation choices
                ...
                val_cont = β * (γ_eul + log(sum(exp.(v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e_next, i_ξ, i_j+1, t+1, :])))) #add continuation value
                if i_ξ == 1 && i_j == J #first time teaching
                    val_cont = 0
                    for i_ξ_next = 2:nξ #loop over potential teacher quality realizations
                        val_cont += (β/nξ) * (γ_eul + log(sum(exp.(v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e_next, i_ξ_next, i_j+1, t+1, :])))) #add continuation value
                    end
                end

                if MA == 1 #already have a masters
                    val_cont = β * v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e_next, i_ξ, i_j+1, t+1, 1]
                    if i_ξ == 1 && i_j == J #first time teaching
                        val_cont = 0
                        for i_ξ_next = 2:nξ #loop over potential teacher quality realizations
                            val_cont +=  (β/nξ) * v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e_next, i_ξ_next, i_j+1, t+1, 1] #add continuation value
                        end
                    end
                end
                val += val_cont
            ...
            val_nwork = β * (γ_eul + log(sum(exp.(v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, 1, t+1, :]))))
            if MA == 1
                val_nwork = β * v_work_a[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, 1, t+1, 1]
            end
            res.v_work_d[i_x, i_χ, i_m, i_MA, i_l, i_e, i_ξ, i_d, i_𝒥, t, 1] = val_nwork #update home work option
        end
    end
end
########
